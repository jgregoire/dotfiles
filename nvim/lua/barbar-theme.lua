...
return {
  ...
  setup = function()
    local fg_current = hl.fg_or_default({'Normal'}, '#f9f7f3', 255)
    local fg_inactive = hl.fg_or_default({'TabLineFill'}, '#3a4055', 102)
    --- @type barbar.utils.hl.group
    local fg_target = {gui = 'red'}
    fg_target.cterm = fg_target.gui

    local fg_error = hl.fg_or_default({'ErrorMsg'}, '#da4939', 124)
    local fg_hint = hl.fg_or_default({'HintMsg'}, '#519f50', 168)
    local fg_info = hl.fg_or_default({'InfoMsg'}, '#b6b3eb', 217)
    local fg_warn = hl.fg_or_default({'WarningMsg'}, '#cc7833', 208)

    local fg_modified = hl.fg_or_default({'WarningMsg'}, '#ffc66d', 178)
    local fg_special = hl.fg_or_default({'Special'}, '#6d9cbe', 75)
    local fg_subtle = hl.fg_or_default({'NonText', 'Comment'}, '#5a647e', 240)
